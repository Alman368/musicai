...
class TransformerEncoderRPR(Module):
    ...
    def forward(self, src, mask=None, src_key_padding_mask=None, is_causal=None):
        """
        Forward pass with optional is_causal parameter for PyTorch compatibility.

        Args:
            src: input tensor
            mask: attention mask
            src_key_padding_mask: padding mask
            is_causal: whether to use causal masking (ignored, mask is used instead)
        """
        output = src
...
def _skew(qe):
    ...
    qe shape: [batch, sz, sz] where sz is sequence length
    After padding: [batch, sz, sz+1]
    After slicing: [batch, sz, sz] by removing leftmost column
    ----------
    """
    ...
    qe = mask * qe
    qe = F.pad(qe, (1, 0, 0, 0, 0, 0))  # Pad 1 column on the left

    # Remove the leftmost column (index 0) by slicing from index 1 onwards
    # This keeps all sz rows but only sz columns (removing the padded column)
    srel = qe[:, :, 1:]
    return srel
