...
class MusicTransformer(nn.Module):
    ...
    def forward(self, x, mask=True):
        ...
        if mask is True:
            mask = self.transformer.generate_square_subsequent_mask(x.shape[1]).to(
                get_device()
            )
        else:
            mask = None
        ...
        while cur_i < target_seq_length:
            ...
            if beam_ran <= beam_chance:
                ...
                if next_token == TOKEN_END:
                    print(
                        "Model called end of sequence at:",
                        cur_i,
                        "/",
                        target_seq_length,
                    )
...
class DummyDecoder(nn.Module):
    ...
    def forward(
        ...
        """
        ----------
        Modified for PyTorch compatibility
        ----------
        Returns the input (memory)

        Additional parameters (tgt_is_causal, memory_is_causal) added for
